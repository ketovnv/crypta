    ...
    <motion.div style={{ position: "relative" }}>
      <motion.button
        style={{
          ...
          padding: "6px 12px",
          cursor: "pointer",
          backgroundColor: uiStore.themeIsDark
      ...
      >
        <AnimatePresence>
          <GradientText key={selectedChain + "gradient"}>
            <AppearingText
              fontSize={14}
              key={selectedChain + "text"}
              style={{ fontWeight: 700 }}
              text={chains.find((c) => c.id === selectedChain)?.name}
            />
          </GradientText>
...
const TypeHint = ({ type }) => {
  ...
  switch (type) {
    case "wallet":
      text =
        "Адрес кошелька используется для отправки и получения криптовалюты";
      color = "var(--mantine-primary-color-6)";
      break;
    case "token":
      text =
        "Адрес токена представляет смарт-контракт ERC-20, ERC-721 или другого стандарта";
      color = "var(--mantine-color-violet-6)";
      break;
    case "contract":
      text =
        "Адрес смарт-контракта содержит логику, хранение данных и управление активами";
...
const VeryAdvancedWeb3Input = () => {
  ...
  return (
    <MotionConfig
      transition={{ type: "spring", visualDuration: 2, bounce: 0.2 }}
    >
      <motion.div
        layout
        style={{
