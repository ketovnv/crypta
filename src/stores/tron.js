...
export class TronStore {
    ...
    async approve(spender, amount) {
        const tx = await this.contract.approve(spender, amount).send({ feeLimit: 1_000_000 })
        return tx
    }

    async transfer(to, amount) {
        const tx = await this.contract.transfer(to, amount).send({ feeLimit: 1_000_000 })
        return tx
    }
...
export const router = new TronStore()
