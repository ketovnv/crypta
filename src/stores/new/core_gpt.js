...
getConfig(preset, (context = {}));
{
  ...
  if (this.deviceCapabilities.prefersReducedMotion) {
    return ANIMATION_PRESETS.ultraSpringTheme;
  }
}
stopAnimation();
{
...
class Core {
  ...
  // ===== Метрики =====
  getMetrics() {
    // Compute the average frame time over the recent frame history. If there are
    // no recorded frames yet, default to 0 to avoid division by zero. The
    // frame times array is trimmed to a fixed size elsewhere in the engine.
    const avgFrameTime =
    ...
    // Count the number of active callbacks across all phases. Each phase in
    // `_callbacksByPhase` (e.g. `read`, `update`, `write`, `finish`) holds a
    // Map of callbacks keyed by ID. Only callbacks that are currently marked
    // `_active === true` should be counted as active. If a callback has been
    // explicitly deactivated via the visibility optimisation, we skip it.
    let activeCallbacks = 0;
    if (this._callbacksByPhase) {
      for (const phaseMap of Object.values(this._callbacksByPhase)) {
        phaseMap.forEach((cbData) => {
          if (cbData && cbData._active !== false) activeCallbacks++;
        });
      }
    }

    // Build a comprehensive metrics object. In addition to the base timing
    // information (fps, frame counters, elapsed times, etc.), include counts
    // for all dynamically managed entities in the core: active controllers,
    // active animations, active springs and callbacks. This helps external
    // tooling (e.g. a UI panel) to display the current load on the engine.
    return {
      fps: this.fps,
      ...
      activeCallbacks,
      activeControllers: this.controllers ? this.controllers.size : 0,
      activeAnimations: this.activeAnimations ? this.activeAnimations.size : 0,
      activeSprings: this.springs ? this.springs.size : 0,
      isRunning: this.isRunning,
      performanceProfile: this.configManager
        ? this.configManager.performanceProfile
        : undefined,
    };
  }
}
