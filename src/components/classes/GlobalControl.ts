...
export class AnimationInterpolator {
    private static cache = new Map<string, OKLCHFast[]>();
    ...
    static init() {
        const createInterpolator = AnimationInterpolator.createStringInterpolator;

        Globals.assign({
            createStringInterpolator: createInterpolator,
            // createStringInterpolator: this.createStringInterpolator,
        });
    }
        ...
        const parsed = colors.map((color) => {
            if (color.startsWith("oklch(")) {
                const parts = color
                    ...
                    .split(/[ ,/]+/)
                    .filter(Boolean);
                // Ensure exactly 4 elements: [L, C, H, Alpha]
                const l = parseFloat(parts[0] ?? "0.5");
                const c = parseFloat(parts[1] ?? "0.05");
                const h = parseFloat(parts[2] ?? "0");
                const a = parts[3] !== undefined ? parseFloat(parts[3]) : 1;
                return [l, c, h, a] as OKLCHFast;
            }
            return [0.5, 0.05, 0, 1] as OKLCHFast; // Fallback
        });
